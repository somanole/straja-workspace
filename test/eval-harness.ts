/**
 * Evaluation Harness for Straja Vault Search
 *
...
function runSearch(query: string): SearchResult[] {
  try {
    const output = execSync(
      `bun src/vault.ts search "${query.replace(/"/g, '\\"')}" --json -n 5 2>/dev/null`,
...
function runQuery(query: string): SearchResult[] {
  try {
    const output = execSync(
      `bun src/vault.ts query "${query.replace(/"/g, '\\"')}" --json -n 5 2>/dev/null`,
...
// Main
console.log("Straja Vault Evaluation Harness");
console.log("=".repeat(50));
console.log(`Testing ${evalQueries.length} queries across 6 documents`);

// Check if eval-docs collection exists
try {
  const status = execSync("bun src/vault.ts status --json 2>/dev/null", { encoding: "utf-8" });
  if (!status.includes("eval-docs")) {
    console.log("\n⚠️  eval-docs collection not found. Run:");
    console.log("   straja-vault collection add test/eval-docs --name eval-docs");
    console.log("   straja-vault embed");
    process.exit(1);
  }
} catch {
  console.log("\n⚠️  Could not check status. Make sure straja-vault is working.");
}
