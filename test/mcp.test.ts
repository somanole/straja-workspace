...
describe("MCP Server", () => {
  beforeAll(async () => {
    ...
    const configPrefix = join(tmpdir(), `vault-mcp-config-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    testConfigDir = await mkdtemp(configPrefix);
    process.env.VAULT_CONFIG_DIR = testConfigDir;
    ...
    testDbPath = `/tmp/vault-mcp-test-${Date.now()}.sqlite`;
  ...
  afterAll(async () => {
    ...
    delete process.env.VAULT_CONFIG_DIR;
  });

  // ===========================================================================
  // Tool: vault_search (BM25)
  // ===========================================================================
  ...
  describe.skipIf(!!process.env.CI)("hybridQuery (expansion + reranking)", () => {
    ...
    test("full hybrid search pipeline", async () => {
      // Simulate full vault deep search flow with type-routed queries
  ...
  // ===========================================================================
  // Tool: vault_get (Get Document)
  // ===========================================================================

  describe("vault_get tool", () => {
  ...
  // ===========================================================================
  // Tool: vault_multi_get (Multi Get)
  // ===========================================================================

  describe("vault_multi_get tool", () => {
  ...
  describe("vault:// resource", () => {
    ...
    test("reads document by display_path", () => {
      ...
      const doc = testDb.prepare(`
        SELECT 'vault://' || d.collection || '/' || d.path as filepath, d.path as display_path, content.doc as body
        FROM documents d
  ...
  describe("MCP spec compliance", () => {
    ...
    test("embedded resources include name and title", () => {
      // Simulate what vault_get returns
      const meta = findDocument(testDb, "readme.md", { includeBody: false });
      expect("error" in meta).toBe(false);
      if ("error" in meta) return;
      const body = getDocumentBody(testDb, meta) ?? "";
      const resource = {
        uri: `vault://${meta.displayPath}`,
...
describe("MCP HTTP Transport", () => {
  ...
  beforeAll(async () => {
    ...
    httpTestDbPath = `/tmp/vault-mcp-http-test-${Date.now()}.sqlite`;
    ...
    const configPrefix = join(tmpdir(), `vault-mcp-http-config-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  test("GET /health returns 200 with status and uptime", async () => {
    ...
    expect(body.httpAuth?.adminRequired).toBe(false);
  });

  test("GET /other returns 404 or SPA fallback", async () => {
    const res = await fetch(`${baseUrl}/other`);
    // If web UI is built, SPA fallback serves index.html (200).
    // If web UI is not built, returns 404.
  ...
  test("POST /mcp initialize returns 200 JSON (not SSE)", async () => {
    ...
    expect(json.result.serverInfo.name).toBe("straja-vault");
  });
  ...
  test("POST /mcp tools/list returns registered tools", async () => {
    ...
    expect(toolNames).toContain("multi_get");
  ...
  test("memory_write creates a new memory document", async () => {
    // Initialize
    await mcpRequest({
      jsonrpc: "2.0", id: 1, method: "initialize",
      params: { protocolVersion: "2025-03-26", capabilities: {}, clientInfo: { name: "test", version: "1.0" } },
    ...
    const { status, json } = await mcpRequest({
      ...
      params: {
        name: "memory_write",
        arguments: { path: "test/fact.md", content: "# Test Fact\n\nThe sky is blue." },
      },
    });
    expect(status).toBe(200);
    expect(json.result).toBeDefined();
    expect(json.result.content[0].text).toContain("Wrote");
    expect(json.result.content[0].text).toContain("_memory/test/fact.md");

    // Verify the document is readable via REST
    const readRes = await fetch(`${baseUrl}/raw/_memory/test/fact.md`);
    expect(readRes.status).toBe(200);
    const body = await readRes.text();
    expect(body).toContain("The sky is blue.");
  });

  test("memory_write appends to existing document", async () => {
    // Initialize
    await mcpRequest({
      jsonrpc: "2.0", id: 1, method: "initialize",
      params: { protocolVersion: "2025-03-26", capabilities: {}, clientInfo: { name: "test", version: "1.0" } },
    ...
    await mcpRequest({
      ...
      params: {
        name: "memory_write",
        arguments: { path: "test/append.md", content: "Line one" },
      },
    });
    ...
    const { status, json } = await mcpRequest({
      ...
      params: {
        name: "memory_write",
        arguments: { path: "test/append.md", content: "Line two", append: true },
      },
    });
    expect(status).toBe(200);
    expect(json.result.content[0].text).toContain("Appended to");

    // Verify both lines are present
    const readRes = await fetch(`${baseUrl}/raw/_memory/test/append.md`);
    const body = await readRes.text();
    expect(body).toContain("Line one");
    expect(body).toContain("Line two");
  });

  test("memory_write overwrites existing document when append is false", async () => {
    // Initialize
    await mcpRequest({
      jsonrpc: "2.0", id: 1, method: "initialize",
      params: { protocolVersion: "2025-03-26", capabilities: {}, clientInfo: { name: "test", version: "1.0" } },
    ...
    await mcpRequest({
      ...
      params: {
        name: "memory_write",
        arguments: { path: "test/overwrite.md", content: "Original content" },
      },
    });
    ...
    await mcpRequest({
      ...
      params: {
        name: "memory_write",
        arguments: { path: "test/overwrite.md", content: "Replaced content" },
      },
    });

    // Verify only new content is present
    const readRes = await fetch(`${baseUrl}/raw/_memory/test/overwrite.md`);
    const body = await readRes.text();
    expect(body).toContain("Replaced content");
    expect(body).not.toContain("Original content");
  });

  test("memory_write document is searchable via query", async () => {
    // Initialize
    await mcpRequest({
      jsonrpc: "2.0", id: 1, method: "initialize",
      params: { protocolVersion: "2025-03-26", capabilities: {}, clientInfo: { name: "test", version: "1.0" } },
    ...
    // Write a document with unique content
    await mcpRequest({
      jsonrpc: "2.0", id: 15, method: "tools/call",
      params: {
        name: "memory_write",
        arguments: { path: "test/searchable.md", content: "xylophone orchestration technique" },
      },
    });

    // Search for it via lex query
    const { json } = await mcpRequest({
      jsonrpc: "2.0", id: 16, method: "tools/call",
      params: {
        name: "query",
        arguments: { searches: [{ type: "lex", query: "xylophone" }] },
      },
    });

    // Should find the memory document
    expect(json.result.content[0].text).toContain("xylophone");
  });

  // ---------------------------------------------------------------------------
  // Browser hardening Phase 0 (HTTP boundary / storage boundary)
  // ---------------------------------------------------------------------------

  test("GET /connections/browser/policy returns default deny policy", async () => {
    const res = await fetch(`${baseUrl}/connections/browser/policy`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.allowedDomains).toEqual([]);
    expect(body.uploadsEnabled).toBe(false);
    expect(body.allowedUploadCollections).toEqual(["_uploads"]);
  });

  test("POST /connections/browser/tool rejects hidden Playwright tools before browser call", async () => {
    const res = await fetch(`${baseUrl}/connections/browser/tool`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "browser_file_upload", arguments: { paths: ["/tmp/secret.txt"] } }),
    });
  ...
  test("PUT /raw/_audit/... is rejected (immutable audit collection)", async () => {
    const res = await fetch(`${baseUrl}/raw/_audit/browser-2026-02-25.jsonl`, {
      method: "PUT",
      body: "{\"tamper\":true}\n",
    });
    expect(res.status).toBe(403);
    const body = await res.json();
    expect(body.error).toContain("_audit");
  });

  test("PUT /raw/_uploads/... is rejected (write-protected upload staging collection)", async () => {
    const res = await fetch(`${baseUrl}/raw/_uploads/sample.txt`, {
      method: "PUT",
      body: "tamper",
    });
    expect(res.status).toBe(403);
    const body = await res.json();
    expect(body.error).toContain("_uploads");
  });

  test("POST /collections rejects reserved _uploads collection", async () => {
    const res = await fetch(`${baseUrl}/collections`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "_uploads", path: "/tmp/any" }),
    });
    expect(res.status).toBe(403);
    const body = await res.json();
    expect(body.error).toContain("system-managed");
    expect(body.error).toContain("_uploads");
  });

  test("GET /status includes system-managed _uploads collection", async () => {
    const res = await fetch(`${baseUrl}/status`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(Array.isArray(body.collections)).toBe(true);
    expect(body.collections.some((c: any) => c.name === "_uploads")).toBe(true);
  });

  test("POST /exec rejects collection=_uploads before exec backend", async () => {
    const res = await fetch(`${baseUrl}/exec`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ command: "echo", args: ["hi"], collection: "_uploads" }),
    });
    expect(res.status).toBe(403);
    const body = await res.json();
    expect(body.error).toContain("system-managed");
  ...
  test("DELETE /collections/_uploads rejects deleting system-managed _uploads collection", async () => {
    const res = await fetch(`${baseUrl}/collections/_uploads`, {
      method: "DELETE",
    });
    expect(res.status).toBe(403);
    const body = await res.json();
  ...
  test("POST /connections/browser/uploads/stage stages file into _uploads collection", async () => {
    const res = await fetch(`${baseUrl}/connections/browser/uploads/stage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        path: "docs/test.txt",
        contentBase64: Buffer.from("hello upload", "utf8").toString("base64"),
        mimeType: "text/plain",
      }),
    });
    ...
    expect(body.collection).toBe("_uploads");
    expect(body.path).toBe("docs/test.txt");
    expect(body.size).toBe(Buffer.byteLength("hello upload", "utf8"));

    const listRes = await fetch(`${baseUrl}/collections/_uploads/files`);
    expect(listRes.status).toBe(200);
    const files = await listRes.json();
    expect(Array.isArray(files)).toBe(true);
    expect(files.some((f: any) => f.path === "docs/test.txt")).toBe(true);
  });

  test("POST /connections/browser/uploads/stage rejects overwrite unless requested", async () => {
    const payload = {
      path: "conflict.txt",
      contentBase64: Buffer.from("one", "utf8").toString("base64"),
    };
    const first = await fetch(`${baseUrl}/connections/browser/uploads/stage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const conflict = await fetch(`${baseUrl}/connections/browser/uploads/stage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  test("POST /connections/browser/uploads/stage-from-media stages vault media into _uploads", async () => {
    const mediaRes = await fetch(`${baseUrl}/media`, {
      method: "POST",
      headers: {
        "Content-Type": "image/png",
        "X-Original-Name": "photo.png",
      },
      body: Buffer.from("png-bytes", "utf8"),
    });
    expect(mediaRes.status).toBe(201);
    const mediaBody = await mediaRes.json();

    const res = await fetch(`${baseUrl}/connections/browser/uploads/stage-from-media`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        mediaUrl: `${baseUrl}/media/${mediaBody.path}`,
        path: "telegram/photo.png",
      }),
    });
    expect([200, 201]).toContain(res.status);
    const body = await res.json();
    expect(body.ok).toBe(true);
    expect(body.collection).toBe("_uploads");
    expect(body.path).toBe("telegram/photo.png");
    expect(body.sourceCollection).toBe("_media");
    expect(body.sourcePath).toBe(mediaBody.path);

    const rawRes = await fetch(`${baseUrl}/raw/_uploads/${encodeURIComponent("telegram/photo.png")}`);
    expect(rawRes.status).toBe(200);
    const rawText = await rawRes.text();
    expect(rawText).toBe("png-bytes");
  });

  test("POST /connections/browser/upload auto-stages vault media before upload attempt", async () => {
    const mediaRes = await fetch(`${baseUrl}/media`, {
      method: "POST",
      headers: {
        "Content-Type": "image/jpeg",
        "X-Original-Name": "facebook.jpg",
      },
      body: Buffer.from("jpeg-bytes", "utf8"),
    });
    expect(mediaRes.status).toBe(201);
    const mediaBody = await mediaRes.json();

    const uploadRes = await fetch(`${baseUrl}/connections/browser/upload`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        mediaUrl: `${baseUrl}/media/${mediaBody.path}`,
        stagePath: "facebook/facebook.jpg",
      }),
    });
    expect(uploadRes.status).toBe(503);

    const rawRes = await fetch(`${baseUrl}/raw/_uploads/${encodeURIComponent("facebook/facebook.jpg")}`);
    expect(rawRes.status).toBe(200);
    const rawText = await rawRes.text();
    expect(rawText).toBe("jpeg-bytes");
  });
});
