 ...
 *
 * Tests all straja-vault CLI commands using a temporary test database via INDEX_PATH.
 * These tests spawn actual straja-vault processes to verify end-to-end functionality.
 */
...
const qmdScript = join(projectRoot, "src", "vault.ts");
...
// Helper to run straja-vault command with test database
async function runQmd(
  ...
  const proc = spawn(tsxBin, [qmdScript, ...args], {
    ...
    env: {
      ...process.env,
      INDEX_PATH: dbPath,
      VAULT_CONFIG_DIR: configDir, // Use test config directory
...
`
  ...
  // Warm up tsx: first spawn compiles TypeScript and populates the cache.
  // Without this, whichever test runs first pays the cold-start cost (30-60s)
  // and may hit the default timeout on slow CI runners.
  await runQmd(["--help"]);
}, 120_000);
...
describe("CLI Help", () => {
  test("shows help with --help flag", async () => {
    ...
    expect(stdout).toContain("straja-vault collection add");
    expect(stdout).toContain("straja-vault search");
  });
...
describe("CLI Context Management", () => {
  ...
  test("add context to virtual path", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toContain("✓ Added context for: vault://fixtures/notes");
  });
  ...
  test("remove virtual path context", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toContain("✓ Removed context for: vault://fixtures/notes");
  });
...
describe("CLI ls Command", () => {
  ...
  test("lists all collections", async () => {
    ...
    expect(stdout).toContain("vault://fixtures/");
  });
  ...
  test("lists files in a collection", async () => {
    ...
    expect(stdout).toContain("vault://fixtures/readme.md");
    expect(stdout).toContain("vault://fixtures/notes/meeting.md");
  });
  ...
  test("lists files with path prefix", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toContain("vault://fixtures/notes/meeting.md");
    expect(stdout).toContain("vault://fixtures/notes/ideas.md");
    // Should not include files outside the prefix (handelize converts to lowercase)
    expect(stdout).not.toContain("vault://fixtures/readme.md");
  });

  test("lists files with virtual path", async () => {
    const { stdout, exitCode } = await runQmd(["ls", "vault://fixtures/docs"], { dbPath: localDbPath });
    expect(exitCode).toBe(0);
    expect(stdout).toContain("vault://fixtures/docs/api.md");
  });
...
describe("CLI Collection Commands", () => {
  ...
  test("renames a collection", async () => {
    // First verify the collection exists
    const { stdout: listBefore } = await runQmd(["collection", "list"], { dbPath: localDbPath });
    expect(listBefore).toContain("vault://fixtures/");
    ...
    expect(stdout).toContain("vault://fixtures/");
    expect(stdout).toContain("vault://my-fixtures/");

    // Verify the new name exists and old name is gone
    const { stdout: listAfter } = await runQmd(["collection", "list"], { dbPath: localDbPath });
    expect(listAfter).toContain("vault://my-fixtures/");
    expect(listAfter).not.toContain("vault://fixtures/"); // Old collection should not appear
  });
  ...
  test("handles renaming to existing collection name", async () => {
    ...
    const tempDir = await mkdtemp(join(tmpdir(), "vault-second-"));
    ...
    expect(listBoth).toContain("vault://fixtures/");
    expect(listBoth).toContain("vault://second/");
...
// =============================================================================
// Output Format Tests - vault:// URIs, context, and docid
// =============================================================================
...
describe("search output formats", () => {
  ...
  beforeAll(async () => {
    ...
    // Add context
    await runQmd(["context", "add", `vault://${collName}/`, "Test fixtures for QMD"], { dbPath: localDbPath, configDir: localConfigDir });
  });

  test("search --json includes vault:// path, docid, and context", async () => {
    ...
    expect(result.file).toMatch(new RegExp(`^vault://${collName}/`));
  ...
  test("search --files includes vault:// path, docid, and context", async () => {
    const { stdout, exitCode } = await runQmd(["search", "test", "--files", "-n", "1"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);

    // Format: #docid,score,vault://collection/path,"context"
    expect(stdout).toMatch(new RegExp(`^#[a-f0-9]{6},[\\d.]+,vault://${collName}/`, "m"));
  ...
  test("search --csv includes vault:// path, docid, and context", async () => {
    ...
    // Data rows should have vault:// paths and context
    expect(stdout).toMatch(new RegExp(`#[a-f0-9]{6},[\\d.]+,vault://${collName}/`));
    expect(stdout).toContain("Test fixtures for QMD");
    // Ensure no full filesystem paths
    expect(stdout).not.toMatch(/\/Users\//);
    expect(stdout).not.toMatch(/\/home\//);
  ...
  test("search --xml includes vault:// path, docid, and context", async () => {
    const { stdout, exitCode } = await runQmd(["search", "test", "--xml", "-n", "1"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);

    expect(stdout).toMatch(new RegExp(`<file docid="#[a-f0-9]{6}" name="vault://${collName}/`));
    expect(stdout).toContain('context="Test fixtures for QMD"');
    // Ensure no full filesystem paths
    expect(stdout).not.toMatch(/\/Users\//);
    expect(stdout).not.toMatch(/\/home\//);
  ...
  test("search default CLI format includes vault:// path, docid, and context", async () => {
    const { stdout, exitCode } = await runQmd(["search", "test", "-n", "1"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);

    // First line should have vault:// path and docid
    expect(stdout).toMatch(new RegExp(`^vault://${collName}/.*#[a-f0-9]{6}`, "m"));
    expect(stdout).toContain("Context: Test fixtures for QMD");
    // Ensure no full filesystem paths
    expect(stdout).not.toMatch(/\/Users\//);
    expect(stdout).not.toMatch(/\/home\//);
...
describe("get command path normalization", () => {
  ...
  test("get with vault://collection/path format", async () => {
    const { stdout, exitCode } = await runQmd(["get", `vault://${collName}/test1.md`, "-l", "3"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);
  ...
  test("get with vault:////collection/path format (extra slashes)", async () => {
    const { stdout, exitCode } = await runQmd(["get", `vault:////${collName}/test1.md`, "-l", "3"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);
    expect(stdout).toContain("Test Document 1");
  });
  ...
  test("get with vault://path:line format", async () => {
    const { stdout, exitCode } = await runQmd(["get", `vault://${collName}/test1.md:3`, "-l", "2"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);
    // Should start from line 3, not line 1
    expect(stdout).not.toMatch(/^# Test Document 1$/m);
  });
...
describe("status and collection list hide filesystem paths", () => {
  ...
  test("status does not show full filesystem paths", async () => {
    ...
    // Should show vault:// URIs
    expect(stdout).toContain(`vault://${collName}/`);
...
describe("mcp http daemon", () => {
  ...
  let daemonCacheDir: string; // XDG_CACHE_HOME value (the straja-vault/ subdir is created automatically)
  ...
  function pidPath(): string {
    return join(daemonCacheDir, "straja-vault", "mcp.pid");
  }

  /** Run straja-vault with test-isolated env (cache, db, config) */
  ...
  function spawnHttpServer(port: number): import("child_process").ChildProcess {
    const proc = spawn(tsxBin, [qmdScript, "mcp", "--http", "--port", String(port)], {
      ...
      env: {
        ...
        VAULT_CONFIG_DIR: daemonConfigDir,
      },
      stdio: ["ignore", "pipe", "pipe"],
    });
  ...
  beforeAll(async () => {
    daemonTestDir = await mkdtemp(join(tmpdir(), "vault-daemon-test-"));
    ...
    await mkdir(join(daemonCacheDir, "straja-vault"), { recursive: true });
