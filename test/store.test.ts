/**
 * store.test.ts - Comprehensive unit tests for the Straja Vault store module
 *
...
async function cleanupTestDb(store: Store): Promise<void> {
  try {
    ...
    // Ignore duplicate close attempts during cleanup paths.
  }
  try {
...
beforeAll(async () => {
  testDir = await mkdtemp(join(tmpdir(), "vault-test-"));
});
...
describe("FTS Search", () => {
  ...
  test("searchFTS finds documents by keyword", async () => {
    ...
    expect(results[0]!.filepath).toBe(`vault://${collectionName}/test/doc1.md`);
  ...
  test("searchFTS ignores inactive documents", async () => {
    ...
    expect(results[0]!.filepath).toBe(`vault://${collectionName}/test/active.md`);
  ...
  describe("findDocument", () => {
    test("findDocument finds by exact filepath", async () => {
      ...
      if (!("error" in result)) {
        ...
        expect(result.filepath).toBe(`vault://${collectionName}/mydoc.md`);
...
describe("Index Status", () => {
  ...
  test("getHashesNeedingEmbedding ignores live session collections", async () => {
    const store = await createTestStore();
    const collectionName = await createTestCollection();
    ...
    await insertTestDocument(store.db, "_sessions", {
      displayPath: "session-1.jsonl",
      body: '{"type":"message","message":{"role":"assistant","content":"session transcript"}}\n',
    });
    await insertTestDocument(store.db, "_sessions_store", {
      displayPath: "sessions.json",
      body: '{"sessions":[{"id":"session-1"}]}',
    });
    await insertTestDocument(store.db, "_orchestration", {
      displayPath: "runs/run-1.json",
      body: '{"traceId":"run-1","status":"dispatch_completed"}',
    });

    expect(store.getHashesNeedingEmbedding()).toBe(1);
    expect(store.getHashesForEmbedding().map((row) => row.path)).toEqual(["test/doc1.md"]);
...
describe("Integration", () => {
  ...
  test("multiple stores can operate independently", async () => {
    ...
    expect(results1[0]!.filepath).toBe("vault://store1/doc.md");

    expect(results2).toHaveLength(1);
    expect(results2[0]!.displayPath).toBe("store2/doc.md");
    expect(results2[0]!.filepath).toBe("vault://store2/doc.md");
...
describe.skipIf(!!process.env.CI)("LlamaCpp Integration", () => {
  ...
  test("searchVec returns results when vector index exists", async () => {
    ...
    expect(results[0]!.filepath).toBe(`vault://${collectionName}/doc1.md`);
...
describe("normalizeVirtualPath", () => {
  test("already normalized vault:// path passes through", () => {
    expect(normalizeVirtualPath("vault://collection/path.md")).toBe("vault://collection/path.md");
    expect(normalizeVirtualPath("vault://journals/2025-01-01.md")).toBe("vault://journals/2025-01-01.md");
  });

  test("handles //collection/path format (missing vault: prefix)", () => {
    expect(normalizeVirtualPath("//collection/path.md")).toBe("vault://collection/path.md");
    expect(normalizeVirtualPath("//journals/2025-01-01.md")).toBe("vault://journals/2025-01-01.md");
  });

  test("handles vault:// with extra slashes", () => {
    expect(normalizeVirtualPath("vault:////collection/path.md")).toBe("vault://collection/path.md");
    expect(normalizeVirtualPath("vault:///journals/2025-01-01.md")).toBe("vault://journals/2025-01-01.md");
    expect(normalizeVirtualPath("vault:///////archive/file.md")).toBe("vault://archive/file.md");
  });

  test("handles collection root paths", () => {
    expect(normalizeVirtualPath("vault://collection/")).toBe("vault://collection/");
    expect(normalizeVirtualPath("vault://collection")).toBe("vault://collection");
    expect(normalizeVirtualPath("//collection/")).toBe("vault://collection/");
  });

  test("preserves bare collection/path format (not auto-converted)", () => {
    // Bare paths without vault:// or // prefix are NOT converted
...
describe("isVirtualPath", () => {
  test("recognizes vault:// paths", () => {
    expect(isVirtualPath("vault://collection/path.md")).toBe(true);
    expect(isVirtualPath("vault://journals/2025-01-01.md")).toBe(true);
    expect(isVirtualPath("vault://collection")).toBe(true);
  });
...
describe("parseVirtualPath", () => {
  test("parses standard vault:// paths", () => {
    expect(parseVirtualPath("vault://collection/path.md")).toEqual({
      collectionName: "collection",
      path: "path.md",
    });
    expect(parseVirtualPath("vault://journals/2025-01-01.md")).toEqual({
  ...
  test("parses paths with nested directories", () => {
    expect(parseVirtualPath("vault://archive/subfolder/file.md")).toEqual({
  ...
  test("parses collection root paths", () => {
    expect(parseVirtualPath("vault://collection/")).toEqual({
      collectionName: "collection",
      path: "",
    });
    expect(parseVirtualPath("vault://collection")).toEqual({
  ...
  test("parses vault:// with extra slashes (normalizes first)", () => {
    expect(parseVirtualPath("vault:////collection/path.md")).toEqual({
      collectionName: "collection",
      path: "path.md",
    });
...
describe("isDocid", () => {
  ...
  test("rejects file paths", () => {
    ...
    expect(isDocid("path/to/file.md")).toBe(false);
    expect(isDocid("vault://collection/file.md")).toBe(false);
  });
...
describe("secret collection indexing policy", () => {
  test("reopening the store removes legacy FTS and embedding rows for secret collections", async () => {
    const store = await createTestStore();
    let reopened: Store | null = null;

    try {
      const publicBody = "Public roadmap document";
      const secretBody = JSON.stringify({ secret: "legacy-pairing-token" }, null, 2);
      const now = new Date().toISOString();

      const publicHash = await hashContent(publicBody);
      const secretHash = await hashContent(secretBody);

      const publicId = await insertTestDocument(store.db, "docs", {
        displayPath: "public.md",
        body: publicBody,
        hash: publicHash,
        title: "Public",
      });
      const secretId = await insertTestDocument(store.db, "_credentials", {
        displayPath: "telegram-allowFrom.json",
        body: secretBody,
        hash: secretHash,
        title: "Credentials",
      });

      // Simulate legacy rows created before the secret-collection policy existed.
      store.db.prepare(`
        INSERT OR REPLACE INTO documents_fts (rowid, filepath, title, body)
        VALUES (?, ?, ?, ?)
      `).run(secretId, "_credentials/telegram-allowFrom.json", "Credentials", secretBody);
      store.db.prepare(`
        INSERT OR REPLACE INTO documents_fts (rowid, filepath, title, body)
        VALUES (?, ?, ?, ?)
      `).run(publicId, "docs/public.md", "Public", publicBody);
      store.db.prepare(`
        INSERT INTO content_vectors (hash, seq, pos, model, embedded_at)
        VALUES (?, 0, 0, 'embeddinggemma', ?)
      `).run(secretHash, now);

      store.close();
      reopened = createStore(testDbPath);

      const indexedPaths = reopened.db.prepare(`
        SELECT filepath
        FROM documents_fts
        WHERE rowid IN (?, ?)
        ORDER BY filepath
      `).all(publicId, secretId) as Array<{ filepath: string }>;
      expect(indexedPaths.map(row => row.filepath)).toEqual(["docs/public.md"]);

      const secretVectorCount = (
        reopened.db.prepare(`SELECT COUNT(*) as count FROM content_vectors WHERE hash = ?`).get(secretHash) as { count: number }
      ).count;
      expect(secretVectorCount).toBe(0);
    } finally {
      await cleanupTestDb(reopened ?? store);
    }
  });
});

describe("orchestration collection indexing policy", () => {
  test("reopening the store removes legacy FTS and embedding rows for _orchestration", async () => {
    const store = await createTestStore();
    let reopened: Store | null = null;

    try {
      const publicBody = "Public roadmap document";
      const orchestrationBody = JSON.stringify({ traceId: "trace-1", status: "dispatch_completed" }, null, 2);
      const now = new Date().toISOString();

      const publicHash = await hashContent(publicBody);
      const orchestrationHash = await hashContent(orchestrationBody);
      ...
      const orchestrationId = await insertTestDocument(store.db, "_orchestration", {
        displayPath: "runs/trace-1.json",
        body: orchestrationBody,
        hash: orchestrationHash,
        title: "Trace",
      });

      // Simulate legacy rows created before the orchestration exclusion existed.
      store.db.prepare(`
        INSERT OR REPLACE INTO documents_fts (rowid, filepath, title, body)
        VALUES (?, ?, ?, ?)
      `).run(orchestrationId, "_orchestration/runs/trace-1.json", "Trace", orchestrationBody);
      store.db.prepare(`
        INSERT OR REPLACE INTO documents_fts (rowid, filepath, title, body)
        VALUES (?, ?, ?, ?)
      `).run(publicId, "docs/public.md", "Public", publicBody);
      ...
      const indexedPaths = reopened.db.prepare(`
        ...
        WHERE rowid IN (?, ?)
        ORDER BY filepath
      `).all(publicId, orchestrationId) as Array<{ filepath: string }>;
      expect(indexedPaths.map(row => row.filepath)).toEqual(["docs/public.md"]);

      const orchestrationVectorCount = (
        reopened.db.prepare(`SELECT COUNT(*) as count FROM content_vectors WHERE hash = ?`).get(orchestrationHash) as { count: number }
      ).count;
      expect(orchestrationVectorCount).toBe(0);
    } finally {
      await cleanupTestDb(reopened ?? store);
    }
  });
