...
// General-purpose instruct model for RAG answer generation
const DEFAULT_ANSWER_MODEL = "hf:ggml-org/Qwen3-1.7B-GGUF/Qwen3-1.7B-Q4_K_M.gguf";
...
export const DEFAULT_ANSWER_MODEL_URI = DEFAULT_ANSWER_MODEL;

// Local model cache directory
const MODEL_CACHE_DIR = join(homedir(), ".cache", "straja-vault", "models");
...
export class LlamaCpp implements LLM {
  ...
  private answerModel: LlamaModel | null = null;
  ...
  private answerModelLoadPromise: Promise<LlamaModel> | null = null;
  ...
  constructor(config: LlamaCppConfig = {}) {
    ...
    this.answerModelUri = DEFAULT_ANSWER_MODEL;
  ...
  async unloadIdleResources(): Promise<void> {
    ...
    if (this.disposeModelsOnInactivity) {
      ...
      if (this.answerModel) {
        await this.answerModel.dispose();
      ...
      this.answerModelLoadPromise = null;
  ...
  private async ensureLlama(): Promise<Llama> {
    if (!this.llama) {
      ...
      if (preferred) {
        try {
          ...
          process.stderr.write(
            `Straja Vault Warning: ${preferred} reported available but failed to initialize. Falling back to CPU.\n`
          );
        }
      } else {
        llama = await getLlama({ gpu: false, logLevel: LlamaLogLevel.error });
      ...
      if (!llama.gpu) {
        process.stderr.write(
          "Straja Vault Warning: no GPU acceleration, running on CPU (slow). Run 'straja-vault status' for details.\n"
        );
      }
      this.llama = llama;
    }
  ...
  /**
   * Load answer model (lazy) - general instruct model for RAG
   */
  private async ensureAnswerModel(): Promise<LlamaModel> {
    if (!this.answerModel) {
      if (this.answerModelLoadPromise) {
        return await this.answerModelLoadPromise;
      }

      this.answerModelLoadPromise = (async () => {
        const llama = await this.ensureLlama();
        const modelPath = await this.resolveModel(this.answerModelUri);
        const model = await llama.loadModel({ modelPath });
      ...
      try {
        await this.answerModelLoadPromise;
      } finally {
        this.answerModelLoadPromise = null;
      }
    }
    this.touchActivity();
    if (!this.answerModel) {
      throw new Error("Answer model not loaded");
    }
  ...
  async answer(prompt: string, options: GenerateOptions = {}): Promise<GenerateResult | null> {
    this.touchActivity();
    await this.ensureAnswerModel();

    const context = await this.answerModel!.createContext();
    const sequence = context.getSequence();
    const session = new LlamaChatSession({ contextSequence: sequence });
    ...
    try {
      ...
      await session.prompt(prompt, {
        ...
        onTextChunk: (text) => {
          result += text;
          // Detect repetition loops: if the last 200 chars repeat a pattern, stop early
          if (result.length > 400) {
            const tail = result.slice(-200);
            const mid = result.slice(-400, -200);
            // Check if tail is very similar to the preceding block (repetition loop)
            const overlap = tail.split("\n").filter(l => l.trim().length > 10 && mid.includes(l.trim()));
            if (overlap.length >= 3) {
              aborted = true;
      ...
      // AbortError is expected when we stop a repetition loop
      if (!aborted) throw e;
    } finally {
      await context.dispose();
    }

    // If we aborted due to repetition, trim the repeated tail
    if (aborted) {
      ...
      for (const line of lines) {
        const key = line.trim();
        if (key.length > 10 && seen.has(key)) continue; // skip duplicate lines
        seen.add(key);
        trimmed.push(line);
      }
      result = trimmed.join("\n");
