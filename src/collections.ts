 ...
 *
 * This module manages the YAML-based collection configuration at ~/.config/straja-vault/index.yml.
...
export interface Collection {
  ...
  update?: string;           // Optional bash command to run during straja-vault update
 ...
 * Config file will be ~/.config/straja-vault/{indexName}.yml
 */
...
function getConfigDir(): string {
  // Allow override via VAULT_CONFIG_DIR for testing
  if (process.env.VAULT_CONFIG_DIR) {
    return process.env.VAULT_CONFIG_DIR;
  }
  // Respect XDG Base Directory specification (consistent with store.ts)
  if (process.env.XDG_CONFIG_HOME) {
    return join(process.env.XDG_CONFIG_HOME, "straja-vault");
  }
  return join(homedir(), ".config", "straja-vault");
}
...
/**
 * Load configuration from ~/.config/straja-vault/index.yml
...
/**
 * Save configuration to ~/.config/straja-vault/index.yml
 */
