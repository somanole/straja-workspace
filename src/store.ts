...
import { openDatabase, loadSqliteVec, resolveDatabaseEncryptionFromEnv } from "./db.js";
import type { Database } from "./db.js";
import { isVaultLocked, resolveVaultDatabaseEncryption } from "./vault-secrets.js";
import picomatch from "picomatch";
import { createHash } from "crypto";
import { realpathSync, statSync, mkdirSync, existsSync, renameSync } from "node:fs";
import {
  LlamaCpp,
  getDefaultLlamaCpp,
...
import { DEFAULT_SYSTEM_COLLECTIONS } from "./workspace-defaults.js";
...
const SECRET_COLLECTIONS = new Set([
  "_credentials",
  "_auth_profiles",
]);

const NON_SEARCHABLE_COLLECTIONS = new Set([
  "_orchestration",
]);

const NON_EMBEDDABLE_COLLECTIONS = new Set([
  "_flows",
  "_orchestration",
  "_sessions",
  "_sessions_store",
...
function isConfigCollection(name: string): boolean {
  return name.startsWith("_config");
}

function isAuditCollection(name: string): boolean {
  return name.startsWith("_audit");
}

function getSearchableCollectionSql(column: string): string {
  return [
    `${column} NOT LIKE '_config%'`,
    `${column} NOT LIKE '_audit%'`,
    `${column} <> '_credentials'`,
    `${column} <> '_auth_profiles'`,
    ...Array.from(NON_SEARCHABLE_COLLECTIONS, (name) => `${column} <> '${name}'`),
  ].join(" AND ");
}

function getEmbeddableCollectionSql(column: string): string {
  return [
    getSearchableCollectionSql(column),
    ...Array.from(NON_EMBEDDABLE_COLLECTIONS, (name) => `${column} <> '${name}'`),
  ].join(" AND ");
}
...
function safeProcessCwd(): string {
  try {
    ...
    // Ignore uv_cwd failures and fall back to HOME.
  }
  return HOME;
}
...
function resolveWorkingDirectory(): string {
  ...
  if (typeof envPwd === "string" && envPwd.trim().length > 0) {
    return normalizePathSeparators(envPwd);
  }
  return normalizePathSeparators(safeProcessCwd());
}
...
export function resolve(...paths: string[]): string {
  ...
  if (isAbsolutePath(firstPath)) {
    ...
    const pwd = resolveWorkingDirectory();
...
// Flag to indicate production mode (set by vault.ts at startup)
...
export function getDefaultDbPath(indexName: string = "index"): string {
  ...
  const dataDir = process.env.XDG_DATA_HOME || resolve(homedir(), ".local", "share");
  const vaultDataDir = resolve(dataDir, "straja-vault");
  try { mkdirSync(vaultDataDir, { recursive: true }); } catch { }

  // Migrate from old location (~/.cache/straja-vault/) if it exists
  const oldCacheDir = process.env.XDG_CACHE_HOME || resolve(homedir(), ".cache");
  const oldPath = resolve(oldCacheDir, "straja-vault", `${indexName}.sqlite`);
  const newPath = resolve(vaultDataDir, `${indexName}.sqlite`);
  try {
...
/**
 * Normalize explicit virtual path formats to standard vault:// format.
 * Only handles paths that are already explicitly virtual:
 * - vault://collection/path.md (already normalized)
 * - vault:////collection/path.md (extra slashes - normalize)
 * - //collection/path.md (missing vault: prefix - add it)
 *
 * Does NOT handle:
...
export function normalizeVirtualPath(input: string): string {
  ...
  // Handle vault:// with extra slashes: vault:////collection/path -> vault://collection/path
  if (path.startsWith('vault:')) {
    // Remove vault: prefix and normalize slashes
    path = path.slice(6);
  ...
  // Handle //collection/path (missing vault: prefix)
  if (path.startsWith('//')) {
    path = path.replace(/^\/+/, '');
    return `vault://${path}`;
  }
...
/**
 * Parse a virtual path like "vault://collection-name/path/to/file.md"
 * into its components.
 * Also supports collection root: "vault://collection-name/" or "vault://collection-name"
 */
export function parseVirtualPath(virtualPath: string): VirtualPath | null {
  // Normalize the path first
  const normalized = normalizeVirtualPath(virtualPath);

  // Match: vault://collection-name[/optional-path]
  // Allows: vault://name, vault://name/, vault://name/path
  const match = normalized.match(/^vault:\/\/([^\/]+)\/?(.*)$/);
...
export function isVirtualPath(path: string): boolean {
  ...
  // Explicit vault:// prefix (with any number of slashes)
  if (trimmed.startsWith('vault:')) return true;

  // //collection/path format (missing vault: prefix)
...
function rebuildFtsTriggers(db: Database): void {
  ...
  db.exec(`
    ...
    WHEN new.active = 1 AND ${getSearchableCollectionSql("new.collection")}
    BEGIN
  ...
  db.exec(`
    ...
    BEGIN
      ...
      SELECT
        ...
        new.title,
        (SELECT doc FROM content WHERE hash = new.hash)
      WHERE new.active = 1 AND ${getSearchableCollectionSql("new.collection")};
    END
  `);
}

function synchronizeFtsIndex(db: Database): void {
  db.exec(`
    DELETE FROM documents_fts
    WHERE rowid NOT IN (
      SELECT id
      FROM documents
      WHERE active = 1 AND ${getSearchableCollectionSql("collection")}
    )
  `);
  ...
  db.exec(`
    ...
    WHERE d.active = 1 AND ${getSearchableCollectionSql("d.collection")}
  `);
}

function cleanupExcludedEmbeddings(db: Database): void {
  const removableHashSql = `
    SELECT DISTINCT d.hash
    FROM documents d
    WHERE d.active = 1
      AND NOT (${getEmbeddableCollectionSql("d.collection")})
      AND NOT EXISTS (
        SELECT 1
        FROM documents keep
        WHERE keep.hash = d.hash
          AND keep.active = 1
          AND ${getEmbeddableCollectionSql("keep.collection")}
      )
  `;

  db.exec(`
    DELETE FROM content_vectors
    WHERE hash IN (${removableHashSql})
  `);
  ...
  db.exec(`
    ...
    WHERE substr(hash_seq, 1, instr(hash_seq, '_') - 1) IN (${removableHashSql})
  `);
}
...
function initializeDatabase(db: Database): void {
  ...
  // Keep busy_timeout short (200ms) so the synchronous better-sqlite3 calls
  // don't block the Node.js event loop for long. The drain worker retries
  // forever on SQLITE_BUSY, so a short timeout just means "try again next
  // tick" rather than freezing the event loop (and all HTTP handling) for
  // seconds while waiting for the embedding subprocess to release the lock.
  ...
  // Collections are now managed in ~/.config/straja-vault/index.yml
  db.exec(`
  ...
  // Migration: add origin column to track how documents were added (scan, manual, gdrive, api).
  // Sync deactivation only removes origin='scan' docs, protecting manually-added files.
  const docCols = db.prepare(`PRAGMA table_info(documents)`).all() as { name: string }[];
  if (!docCols.some(col => col.name === "origin")) {
    db.exec(`ALTER TABLE documents ADD COLUMN origin TEXT NOT NULL DEFAULT 'scan'`);
  }
  ...
  // Rebuild FTS triggers on startup so old databases adopt the current policy.
  rebuildFtsTriggers(db);
  synchronizeFtsIndex(db);
  cleanupExcludedEmbeddings(db);
}
...
export type Store = {
  ...
  getHealthStats: () => Omit<VaultHealthStats, "writeQueue" | "embedding" | "uptimeSeconds">;
  ...
  writeDocumentAtomic: (collection: string, path: string, title: string, content: string, hash: string, now: string, origin?: string) => { action: "created" | "updated" };
  insertContent: (hash: string, content: string, createdAt: string) => void;
  insertDocument: (collectionName: string, path: string, title: string, hash: string, createdAt: string, modifiedAt: string, origin?: string) => void;
  findActiveDocument: (collectionName: string, path: string) => { id: number; hash: string; title: string } | null;
  getDocumentWithContent: (collectionName: string, path: string) => { id: number; hash: string; title: string; content: string } | null;
  updateDocumentTitle: (documentId: number, title: string, modifiedAt: string) => void;
  updateDocument: (documentId: number, title: string, hash: string, modifiedAt: string) => void;
  deactivateDocument: (collectionName: string, path: string) => void;
  getActiveDocumentPaths: (collectionName: string, origin?: string) => string[];
 ...
 * If no path is provided, uses the default path (~/.cache/straja-vault/index.sqlite).
 *
...
export function createStore(dbPath?: string): Store {
  ...
  const envEncryption = resolveDatabaseEncryptionFromEnv();
  if (isVaultLocked(resolvedPath) && !envEncryption) {
    throw new Error("Vault is locked. Unlock it before opening the database.");
  }
  const db = openDatabase(resolvedPath, {
    encryption: envEncryption ?? resolveVaultDatabaseEncryption(resolvedPath),
  });
  ...
  return {
    ...
    getHealthStats: () => getHealthStats(db, resolvedPath),
    ...
    // Document indexing operations
    writeDocumentAtomic: (collection: string, path: string, title: string, content: string, hash: string, now: string, origin?: string) => writeDocumentAtomic(db, collection, path, title, content, hash, now, origin),
    insertContent: (hash: string, content: string, createdAt: string) => insertContent(db, hash, content, createdAt),
    insertDocument: (collectionName: string, path: string, title: string, hash: string, createdAt: string, modifiedAt: string, origin?: string) => insertDocument(db, collectionName, path, title, hash, createdAt, modifiedAt, origin),
    findActiveDocument: (collectionName: string, path: string) => findActiveDocument(db, collectionName, path),
    getDocumentWithContent: (collectionName: string, path: string) => getDocumentWithContent(db, collectionName, path),
    ...
    getActiveDocumentPaths: (collectionName: string, origin?: string) => getActiveDocumentPaths(db, collectionName, origin),
...
// ---------------------------------------------------------------------------
// Path convention: originals vs index entries
// Originals:  "report.pdf", "data.xlsx", "notes.md"
// Index entries: "report.pdf::1", "data.xlsx::text", "data.xlsx::sheet-q1"
// ---------------------------------------------------------------------------

/** Separator between base file path and index entry suffix. */
export const INDEX_SEPARATOR = "::";

/** True if path is an index entry (contains ::). */
export function isIndexPath(path: string): boolean {
  return path.includes(INDEX_SEPARATOR);
}

/** Extract the base file path from an index entry path.
 *  "report.pdf::3" → "report.pdf"
 *  "notes.md" → "notes.md" (already a base path) */
export function getBasePath(path: string): string {
  const i = path.indexOf(INDEX_SEPARATOR);
  return i === -1 ? path : path.slice(0, i);
}

/** Build an index entry path: ("report.pdf", "3") → "report.pdf::3" */
export function buildIndexPath(base: string, suffix: string): string {
  return `${base}${INDEX_SEPARATOR}${suffix}`;
}

/** Binary file extensions that get a separate original + index entries. */
const BINARY_IMPORT_EXTS = new Set([".pdf", ".xlsx", ".xls", ".csv"]);

/** True if the file extension requires dual storage (original blob + text index). */
export function isBinaryImportExt(ext: string): boolean {
  return BINARY_IMPORT_EXTS.has(ext.toLowerCase());
}
...
export function getHashesNeedingEmbedding(db: Database): number {
  const result = db.prepare(`
    ...
    WHERE d.active = 1 AND v.hash IS NULL AND LENGTH(c.doc) > 0
      AND ${getEmbeddableCollectionSql("d.collection")}
      AND c.doc NOT LIKE '{"kind":"straja-vault-browser-upload-blob%'
  `).get() as { count: number };
  return result.count;
}
...
export function cleanupOrphanedVectors(db: Database): number {
  ...
  // Find all orphaned hash+seq pairs
  ...
  `).all() as { hash: string; seq: number }[];

  if (orphanedRows.length === 0) return 0;

  console.log(`[cleanupOrphanedVectors] Found ${orphanedRows.length} orphaned entries to remove`);

  // Delete from vectors_vec in batches using individual DELETEs.
  // sqlite-vec virtual tables don't support subquery-based DELETE,
  // so we must delete one row at a time.
  if (tableExists) {
    ...
    try {
      for (let i = 0; i < orphanedRows.length; i++) {
        const r = orphanedRows[i]!;
        try {
          deleteVecStmt.run(`${r.hash}_${r.seq}`);
        } catch {
          // Individual delete may fail if row already gone — safe to skip
        }
      ...
      console.error(`[cleanupOrphanedVectors] Error deleting from vectors_vec:`, err);
    }
  }
  ...
  console.log(`[cleanupOrphanedVectors] Cleanup complete: ${orphanedRows.length} entries processed`);
  return orphanedRows.length;
}

/**
 * Remove vector embeddings for a specific content hash.
 * Called when a document is deactivated and its hash is no longer referenced by any active document.
 */
export function cleanupVectorsForHash(db: Database, hash: string): void {
  const tableExists = db.prepare(
    `SELECT name FROM sqlite_master WHERE type='table' AND name='vectors_vec'`
  ).get();
  if (!tableExists) return;

  // Get all seq values for this hash to build hash_seq keys
  ...
  const hashSeqs = seqs.map(s => `${hash}_${s.seq}`);
  db.prepare(`DELETE FROM vectors_vec WHERE hash_seq IN (${placeholders})`).run(...hashSeqs);
...
/**
 * Collect all health stats from the database in a single call.
 * All queries are lightweight COUNT(*) — safe for frequent polling.
 */
export function getHealthStats(db: Database, dbPath: string): Omit<VaultHealthStats, "writeQueue" | "embedding" | "uptimeSeconds"> {
  ...
  const docCounts = db.prepare(`
    ...
    FROM documents
  `).get() as { active: number; inactive: number };

  // Total content hashes
  const totalContent = (db.prepare(`SELECT COUNT(*) as c FROM content`).get() as { c: number }).c;

  // Orphaned content hashes (no active doc references)
  const orphanedContent = (db.prepare(`
    SELECT COUNT(*) as c FROM content c
    WHERE NOT EXISTS (SELECT 1 FROM documents d WHERE d.hash = c.hash AND d.active = 1)
  `).get() as { c: number }).c;
  ...
  if (ftsExists) {
    ftsEntries = (db.prepare(`SELECT COUNT(*) as c FROM documents_fts`).get() as { c: number }).c;
  }
  ...
  if (cacheExists) {
    llmCacheEntries = (db.prepare(`SELECT COUNT(*) as c FROM llm_cache`).get() as { c: number }).c;
  }
  ...
  return {
    activeDocuments: docCounts.active,
    inactiveDocuments: docCounts.inactive,
    totalContentHashes: totalContent,
    orphanedContentHashes: orphanedContent,
    activeVectors,
    orphanedVectors,
    needsEmbedding,
    hasVectorIndex,
...
export function insertContent(db: Database, hash: string, content: string, createdAt: string): void {
  // better-sqlite3 has a practical binding size limit; skip oversized content
  const MAX_CONTENT_BYTES = 50 * 1024 * 1024; // 50 MB
  if (Buffer.byteLength(content, "utf-8") > MAX_CONTENT_BYTES) {
    console.warn(`Skipping oversized content (hash=${hash.slice(0, 8)}…, ${(Buffer.byteLength(content, "utf-8") / 1024 / 1024).toFixed(1)} MB)`);
    return;
  }
...
/**
 * Atomic document write: inserts content + inserts/updates document in a single
 * transaction. Eliminates FK ordering issues — either both succeed or both roll
 * back. Used by the write queue drain worker.
 *
 * Uses BEGIN IMMEDIATE to acquire the write lock up front. If the database is
 * busy, SQLITE_BUSY is thrown at BEGIN (before any work), and the drain worker
 * can retry the entire operation safely.
 */
export function writeDocumentAtomic(
  ...
  try {
    ...
    ).get(collection, path) as { id: number } | undefined;

    if (existing) {
      db.prepare(
        `UPDATE documents SET title = ?, hash = ?, modified_at = ? WHERE id = ?`
      ).run(title, hash, now, existing.id);
      db.exec("COMMIT");
      return { action: "updated" };
    }
    ...
    db.prepare(
      `INSERT INTO documents (collection, path, title, hash, created_at, modified_at, active, origin)
       ...
       ON CONFLICT(collection, path) DO UPDATE SET
         ...
         active = 1`
    ).run(collection, path, title, hash, now, now, origin);
    db.exec("COMMIT");
...
export function insertDocument(
  ...
  db.prepare(`
    ...
    ON CONFLICT(collection, path) DO UPDATE SET
      ...
      modified_at = excluded.modified_at,
      active = 1
  `).run(collectionName, path, title, hash, createdAt, modifiedAt, origin);
}
...
/**
 * Get an active document with its content, by collection and path.
 * Joins documents + content tables in a single query.
 */
export function getDocumentWithContent(
  db: Database,
  collectionName: string,
  path: string,
): { id: number; hash: string; title: string; content: string } | null {
  const row = db.prepare(`
    SELECT d.id, d.hash, d.title, c.doc as content
    FROM documents d
    JOIN content c ON c.hash = d.hash
    WHERE d.collection = ? AND d.path = ? AND d.active = 1
  `).get(collectionName, path) as { id: number; hash: string; title: string; content: string } | undefined;
  return row ?? null;
}
 ...
 * Also cleans up vector embeddings for hashes that are no longer referenced by any active document.
 */
export function deactivateDocument(db: Database, collectionName: string, path: string): void {
  // Get the hashes before deactivating so we know what to clean up
  const hashes = db.prepare(
    `SELECT DISTINCT hash FROM documents WHERE collection = ? AND path = ? AND active = 1`
  ).all(collectionName, path) as { hash: string }[];

  db.prepare(`UPDATE documents SET active = 0 WHERE collection = ? AND path = ? AND active = 1`)
    .run(collectionName, path);

  // Clean up vectors for hashes that are now orphaned (no other active doc references them)
  for (const { hash } of hashes) {
    const stillReferenced = db.prepare(
      `SELECT 1 FROM documents WHERE hash = ? AND active = 1 LIMIT 1`
    ).get(hash);
    if (!stillReferenced) {
      cleanupVectorsForHash(db, hash);
    }
  }
}
 ...
 */
export function getActiveDocumentPaths(db: Database, collectionName: string, origin?: string): string[] {
  if (origin) {
    const rows = db.prepare(
      `SELECT path FROM documents WHERE collection = ? AND active = 1 AND origin = ?`
    ).all(collectionName, origin) as { path: string }[];
...
export function findDocumentByDocid(db: Database, docid: string): { filepath: string; hash: string } | null {
  ...
  const doc = db.prepare(`
    SELECT 'vault://' || d.collection || '/' || d.path as filepath, d.hash
    FROM documents d
    WHERE d.hash LIKE ? AND d.active = 1
      AND ${getSearchableCollectionSql("d.collection")}
    LIMIT 1
...
export function matchFilesByGlob(db: Database, pattern: string): { filepath: string; displayPath: string; bodyLength: number }[] {
  const allFiles = db.prepare(`
    SELECT
      'vault://' || d.collection || '/' || d.path as virtual_path,
...
export function getContextForFile(db: Database, filepath: string): string | null {
  ...
  // Parse virtual path format: vault://collection/path
  let collectionName: string | null = null;
  let relativePath: string | null = null;

  const parsedVirtual = filepath.startsWith('vault://') ? parseVirtualPath(filepath) : null;
  if (parsedVirtual) {
 ...
 * Returns collection metadata from ~/.config/straja-vault/index.yml
 */
 ...
 */
export function removeCollection(db: Database, collectionName: string): { deletedDocs: number; cleanedHashes: number; cleanedVectors: number } {
  ...
  const cleanedVectors = cleanupOrphanedVectors(db);
...
export function validateSemanticQuery(query: string): string | null {
  // Check for negation syntax: standalone -term at start or after whitespace
  // (but NOT hyphenated words like "Pre-Collision")
...
export function searchFTS(db: Database, query: string, limit: number = 20, collectionName?: string): SearchResult[] {
  ...
  if (!ftsQuery) return [];
  if (collectionName && !isExternallyVisibleCollection(collectionName)) return [];

  let sql = `
    SELECT
      'vault://' || d.collection || '/' || d.path as filepath,
  ...
  if (collectionName) {
    ...
    sql += ` AND ${getSearchableCollectionSql("d.collection")}`;
  }
...
export async function searchVec(db: Database, query: string, model: string, limit: number = 20, collectionName?: string, session?: ILLMSession, precomputedEmbedding?: number[]): Promise<SearchResult[]> {
  ...
  // Regression test: structured search with all-lex queries
  ...
  if (collectionName) {
    ...
    docSql += ` AND ${getSearchableCollectionSql("d.collection")}`;
  }
...
export async function rerank(query: string, documents: { file: string; text: string }[], model: string = DEFAULT_RERANK_MODEL, db: Database): Promise<{ file: string; score: number }[]> {
  // Skip LLM reranking when models are not available (CI, first run, etc.)
  if (process.env.CI || process.env.VAULT_SKIP_RERANK) {
 ...
 *
 * Supports:
 * - Virtual paths: vault://collection/path/to/file.md
...
export function findDocument(db: Database, filename: string, options: { includeBody?: boolean } = {}): DocumentResult | DocumentNotFound {
  ...
  let doc = db.prepare(`
    ...
    FROM documents d
    JOIN content ON content.hash = d.hash
    WHERE 'vault://' || d.collection || '/' || d.path = ? AND d.active = 1
      AND ${getSearchableCollectionSql("d.collection")}
  ...
  if (!doc) {
    doc = db.prepare(`
      ...
      FROM documents d
      JOIN content ON content.hash = d.hash
      WHERE 'vault://' || d.collection || '/' || d.path LIKE ? AND d.active = 1
        AND ${getSearchableCollectionSql("d.collection")}
      LIMIT 1
    `).get(`%${filepath}`) as DbDocRow | null;
  }
  ...
  if (!doc && !filepath.startsWith('vault://')) {
    const collections = collectionsListCollections();
    for (const coll of collections) {
      let relativePath: string | null = null;
  ...
  const virtualPath = doc.virtual_path || `vault://${doc.collection}/${doc.display_path}`;
...
export function findDocuments(
  ...
  if (isCommaSeparated) {
    ...
    fileRows = db.prepare(`
      ...
      FROM documents d
      JOIN content ON content.hash = d.hash
      WHERE 'vault://' || d.collection || '/' || d.path IN (${placeholders}) AND d.active = 1
        AND ${getSearchableCollectionSql("d.collection")}
  ...
  for (const row of fileRows) {
    // Get context using virtual path
    const virtualPath = row.virtual_path || `vault://${row.collection}/${row.display_path}`;
    const context = getContextForFile(db, virtualPath);
...
/** Collections with this prefix are hidden from all external APIs (MCP, HTTP, UI). */
export function isHiddenCollection(name: string): boolean {
  return isConfigCollection(name) || isAuditCollection(name);
}

/** Secret-bearing collections that must never be indexed or exposed through search/UI. */
export function isSecretCollection(name: string): boolean {
  return isConfigCollection(name) || SECRET_COLLECTIONS.has(name);
}

/** Collections that should be visible in external list/search/browse surfaces. */
export function isExternallyVisibleCollection(name: string): boolean {
  return !isHiddenCollection(name) && !isSecretCollection(name);
}

/** Collections that must never receive embeddings. */
export function isEmbeddingExcludedCollection(name: string): boolean {
  return !isExternallyVisibleCollection(name) || NON_EMBEDDABLE_COLLECTIONS.has(name);
}

/** System-managed collections that cannot be deleted or manually modified by users. */
const SYSTEM_COLLECTIONS = new Set([
  "_uploads", "_screenshots", "_editable",
  "_gmail", "_gdrive", "_calendar", "_contacts",
  "_notes",
]);
export function isSystemCollection(name: string): boolean {
  return SYSTEM_COLLECTIONS.has(name) || name.startsWith("_");
}

/** Collections that external raw write endpoints must reject. */
export function isWriteProtectedCollection(name: string): boolean {
  return name === "_audit" || name === "_uploads" || name === "_write_queue";
}
...
export function getStatus(db: Database): IndexStatus {
  ...
  const yamlCollections = collectionsListCollections();
  const yamlNames = new Set(yamlCollections.map(c => c.name));

  // Get document counts and last update times for each YAML collection
  const collections = yamlCollections.map(col => {
    const stats = db.prepare(`
      SELECT
        COUNT(*) as active_count,
        COUNT(CASE WHEN path NOT LIKE '%::%' AND path != '.collection' AND path NOT LIKE '%/.gitkeep' THEN 1 END) as file_count,
        MAX(modified_at) as last_doc_update
      FROM documents
      WHERE collection = ? AND active = 1
    `).get(col.name) as { active_count: number; file_count: number; last_doc_update: string | null };

    // A collection is "linked" if its path is a real directory on disk
    // (not a virtual path used for add-files or system collections)
    let linked = false;
    try {
      linked = !isSystemCollection(col.name) && existsSync(col.path) && statSync(col.path).isDirectory();
    } catch {}

    // _notes is system-managed (cannot be deleted) but user-facing for UI grouping
    const isSystem = isSystemCollection(col.name);
    const isUserFacing = col.name === "_notes";
    ...
    return {
      ...
      type: (isSystem && !isUserFacing) ? "system" as const : "user" as const,
      linked,
      deletable: !isSystem,
    };
  });

  // Discover "raw-only" collections — created via /raw/ endpoints (e.g. _workspace, _sessions)
  // that don't have YAML config entries but have documents in the database.
  const dbCollections = db.prepare(`
    SELECT collection, COUNT(*) as active_count,
           COUNT(CASE WHEN path NOT LIKE '%::%' AND path != '.collection' AND path NOT LIKE '%/.gitkeep' THEN 1 END) as file_count,
           MAX(modified_at) as last_doc_update
    FROM documents
    WHERE active = 1
    GROUP BY collection
  `).all() as { collection: string; active_count: number; file_count: number; last_doc_update: string | null }[];

  for (const row of dbCollections) {
    if (!yamlNames.has(row.collection)) {
      const isSystem = isSystemCollection(row.collection);
      const isUserFacing = row.collection === "_notes";
      collections.push({
        name: row.collection,
        path: "",
        pattern: "",
  ...
  for (const name of DEFAULT_SYSTEM_COLLECTIONS) {
    if (!collections.some((entry) => entry.name === name)) {
      collections.push({
        name,
        path: "",
        pattern: "",
  ...
  // Filter hidden collections (e.g. _config) from external-facing status
  const visibleCollections = collections.filter(c => isExternallyVisibleCollection(c.name));
  ...
  const totals = db.prepare(`
    SELECT COUNT(*) as c,
           COUNT(CASE WHEN path NOT LIKE '%::%' AND path != '.collection' AND path NOT LIKE '%/.gitkeep' THEN 1 END) as fc
    FROM documents
    WHERE active = 1 AND ${getSearchableCollectionSql("collection")}
  `).get() as { c: number; fc: number };
  const needsEmbedding = getHashesNeedingEmbedding(db);
  const hasVectors = !!db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='vectors_vec'`).get();

  return {
    totalDocuments: totals.c,
    totalFileCount: totals.fc,
    needsEmbedding,
...
export interface HybridQueryResult {
  file: string;             // internal filepath (vault://collection/path)
  displayPath: string;
  title: string;
...
export async function hybridQuery(
  ...
  // Step 6: Rerank chunks (NOT full bodies) — gracefully skip if model unavailable
  let reranked: { file: string; score: number }[];
  try {
    hooks?.onRerankStart?.(chunksToRerank.length);
    const rerankStart = Date.now();
    reranked = await store.rerank(query, chunksToRerank);
    hooks?.onRerankDone?.(Date.now() - rerankStart);
  } catch {
    // Reranker model not available — fall back to RRF-only ordering
    reranked = chunksToRerank.map(c => ({ file: c.file, score: 0.5 }));
  }
...
export async function structuredSearch(
  ...
  const collections = options?.collections?.filter(isExternallyVisibleCollection);
  ...
  // Step 5: Rerank chunks (gracefully skip if reranker model unavailable)
  let reranked: { file: string; score: number }[];
  try {
    hooks?.onRerankStart?.(chunksToRerank.length);
    const rerankStart2 = Date.now();
    reranked = await store.rerank(primaryQuery, chunksToRerank);
    hooks?.onRerankDone?.(Date.now() - rerankStart2);
  } catch {
    // Reranker model not available (e.g., CI, first run before model download).
    // Fall back to RRF-only ordering with uniform reranker scores.
    reranked = chunksToRerank.map(c => ({ file: c.file, score: 0.5 }));
  }

  // Step 6: Blend RRF position score with reranker score
  // The reranker (cross-encoder) evaluates actual query-document relevance,
  // so it gets the dominant weight. RRF position is a retrieval signal that
  // acts as a tiebreaker — documents that appear across multiple retrieval
  // methods get a small boost, but it can't override the reranker's judgment.
  const candidateMap = new Map(candidates.map(c => [c.file, {
    displayPath: c.displayPath, title: c.title, body: c.body,
  }]));
  const rrfRankMap = new Map(candidates.map((c, i) => [c.file, i + 1]));
  ...
  const blended = reranked.map(r => {
    const rrfRank = rrfRankMap.get(r.file) || candidateLimit;
    // Damped RRF position score: rank 1 → 1.0, rank 10 → 0.55, rank 40 → 0.22
    // Gentle decay prevents rank 1 from dominating the final score.
    const K_DAMP = 10;
    const rrfScore = (K_DAMP + 1) / (K_DAMP + rrfRank);
    // Reranker gets 80% weight — it's a cross-encoder that actually reads
    // the query + chunk, so it's far more reliable than retrieval position.
    const rrfWeight = 0.20;
    const blendedScore = rrfWeight * rrfScore + (1 - rrfWeight) * r.score;
